...
public class ScheduledService extends Service
{
    ...
    private void getFriendsInfo(User user) {
        ...
        if(current_fid_list.length<temp_fid_list.length)
        {
            newfid = new String();
            newfname = new String();
            newoid = new String();
            newoname = new String();
            newfnameList = new String[temp_fid_list.length-current_fid_list.length];
            newfidList = new String[temp_fid_list.length-current_fid_list.length];
            for(int i=current_fid_list.length-1; i<temp_fid_list.length; i++) {
                newfnameList[i - current_fid_list.length - 1] = temp_fname_list[i];
                newfidList[i - current_fid_list.length - 1] = temp_fid_list[i];
            }
            if(LocalStoreController.userLocalStore.getString("oid_list", "")!= "") {
